    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* Drawer — slides up from bottom on mobile, in from right on desktop */}
          <motion.div
            initial={{ y: '100%' }}
            animate={{ y: 0 }}
            exit={{ y: '100%' }}
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-white/10 shrink-0">
              ...
              <button
                onClick={onClose}
                className="p-2 rounded-full hover:bg-white/10 text-ash hover:text-crema transition-colors"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 sm:px-6 py-4 space-y-3 min-h-0">
              {cart.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-40 text-center gap-3">
                  ...
                  <p className="text-ash/50 italic text-sm">
                ...
                cart.map(item => (
                  <div
                    key={item.id}
                    className="flex items-center gap-3 bg-white/[0.04] rounded-xl p-3 sm:p-3.5 border border-white/5"
                  >
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button
                        ...
                        className="w-7 h-7 rounded-full bg-white/10 flex items-center justify-center text-ash hover:text-crema hover:bg-white/20 transition-all active:scale-90"
                      >
                        <Minus size={11} />
                      </button>
                      ...
                      <button
                        ...
                        className="w-7 h-7 rounded-full bg-red-950/50 flex items-center justify-center text-red-400 hover:bg-red-900/60 transition-all active:scale-90 ml-0.5"
                      >
                        <X size={11} />
                      </button>
                    </div>

                    <p className="text-crema font-bold text-sm shrink-0 w-14 text-right">
            ...
            {cart.length > 0 && (
              <div className="px-5 sm:px-6 py-4 sm:py-5 border-t border-white/10 shrink-0 space-y-3">
                ...
                <button
                  ...
                  className="w-full bg-amber-gold hover:bg-amber-gold/90 active:bg-amber-gold/80 text-espresso font-bold py-3.5 rounded-full transition-all duration-200 tracking-wide text-sm sm:text-base"
                >
