// src/components/ThreeDImageRing.tsx
"use client";
...
export function ThreeDImageRing({
  ...
  return (
    ...
    >
      <div style={{ perspective: `${perspective}px`, width: `${width}px`, height: `${width * 1.33}px`, position: "absolute", left: "50%", top: "50%", transform: "translate(-50%, -50%)" }}>
        <motion.div
          ref={ringRef}
        ...
        >
          <AnimatePresence>
            {showImages && images.map((imageUrl, index) => (
              <motion.div
                key={index}
                className={cn("w-full h-full absolute rounded-xl border border-white/10", imageClassName)}
                style={{
                ...
                custom={index}
                transition={{ delay: index * staggerDelay, duration: animationDuration }}
